...
"use strict"
var states = require('jst/commons/constants').grammar.selectorStates
...
parsePart.OPERATOR = token => {

}

parsePart.PSEUDOCLASS = token => {

}

parsePart.PSEUDOCLASS_ARGUMENT_SEPERATOR = token => {

}

parsePart.PSEUDOCLASS_ARGUMENTS_CLOSE = token => {

}

parsePart.PSEUDOCLASS_ARGUMENTS_OPEN = token => {

}

parsePart.RULE_SEPERATOR = token => {

}

parsePart.STRING_OPEN = token => {

}

parsePart.STRING = token => {

}

parsePart.TYPE = token => {
...
var parseRules = tokens => {

	var state               = tokens[0].state
	var parts               = [ ]
	var expectedTransitions = selectorGrammar[tokens[0].state]

	for (var ith = 1; ith < tokens.length; ++ith) {

		var token           = tokens[ith]
		state               = tokens[ith].state

		if (expectedTransitions.indexOf(state) === -1) {

			var message = `${state} cannot transition to ${expectedTransitions}: ` + JSON.stringify(token)

			throw Error(message)

		}

		expectedTransitions = selectorGrammar[state]

		parts.push(parsePart(token))
...
//		states.ATTRIBUTE_CLOSE
	...
	return parseRules( translate.checkEnums(filtered))
